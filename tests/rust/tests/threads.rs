...
async fn send_thread_reply_subscriber_receives_event_batch() {
    ...
    // Both connect via WebSocket, authenticate, subscribe
    let mut ws_a = WsTestClient::connect()
        .await
        .expect("WS connect A should succeed");
    ...
    let mut ws_b = WsTestClient::connect()
        .await
        .expect("WS connect B should succeed");
    ...
    // User A sends a thread reply (using a synthetic thread_id for the contract test)
    let fake_thread_id = uuid::Uuid::new_v4().to_string();
    ws_a.send_thread_reply(&channel.channel_id, &fake_thread_id, "Thread reply from A")
        .await
        .expect("send thread reply should succeed");

    // User B should receive an EVENT_BATCH with the thread reply event
    let (_stream_id, event_batch) = ws_b
