...
async fn send_reaction_subscriber_receives_event_batch() {
    ...
    // User A sends a reaction (using a synthetic message_id for the contract test)
    let fake_message_id = uuid::Uuid::new_v4().to_string();
    ws_a.send_reaction(&channel.channel_id, &fake_message_id, "fire", "add")
        .await
