...
test.describe('Messaging', () => {
  test('send a message, refresh, and still see it', async ({ page }) => {
    ...
    await measureE2E(perf, 'workspace.open_default_channel_ui', { phase: 'browse' }, async () => {
    ...
    await measureE2E(perf, 'messaging.send_visible_same_tab_ui', { phase: 'compose' }, async () => {
    ...
    await measureE2E(perf, 'messaging.reload_restore_ui', { phase: 'reload' }, async () => {
      await page.reload();
      await expect(page).toHaveURL(/\/#\/workspace\//);
      await expect(page.locator('.channel-item').filter({ hasText: 'general' })).toBeVisible();
      await page.locator('.channel-item').filter({ hasText: 'general' }).click();
  ...
  test('cross-tab messaging: message sent in one tab appears in another', async ({ browser }) => {
    ...
    await measureE2E(perf, 'messaging.cross_tab_delivery_ui', { phase: 'realtime' }, async () => {
  ...
  test('send multiple messages and verify order', async ({ page }) => {
    ...
    await measureE2E(perf, 'messaging.multi_send_ui', { messages: 3 }, async () => {
      for (let i = 1; i <= 3; i++) {
